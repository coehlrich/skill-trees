...
public abstract class Menu{
    ...
    public final void openAnvilGUI(String initialText, String title, BiConsumer<Player, String> completeFunction){
        close();
        new AnvilGUI.Builder().text(initialText).plugin(SkillTreeCore.INSTANCE).title(title).item(new ItemBuilder(Material.PAPER).setDisplayName(initialText).build()).onClose((plyr) -> {
            new BukkitRunnable() {
                @Override
                public void run(){
                    open(Menu.this);
                }
            }.runTask(SkillTreeCore.INSTANCE);
